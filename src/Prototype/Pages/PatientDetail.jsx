// src/Prototype/Pages/PatientDetail.jsx
// @ts-nocheck
...
export default function PatientDetail() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!famErr) setFamilyMembers(famData || []);
        ...
        if (!healthErr) setHealthRecords(healthData || []);
        ...
        if (!aptErr) setAppointments(aptData || []);
      } catch (err) {
  ...
  // Risk calculation (same logic as your Firebase version)
  const calculateRiskLevel = () => {
    const scdFamilyMembers = familyMembers.filter(
      (m) =>
        m.health_status &&
        m.health_status.toLowerCase().includes("scd")
    );

    const earlyOnset = scdFamilyMembers.filter(
  ...
  const scdEarlyOnset = familyMembers.filter(
    (m) =>
      m.health_status &&
      m.health_status.toLowerCase().includes("scd") &&
      m.diagnosis_age &&
        ...
        {scdEarlyOnset.length > 0 && (
          ...
          >
              ...
              <div className="alert-header-content">
                ...
                <div className="alert-header-info">
                  ...
                  <p className="alert-date">
                    {scdEarlyOnset.length} family member
                    {scdEarlyOnset.length > 1 ? "s" : ""} with SCD diagnosed
                    before age 50
                  </p>
                </div>
              </div>
            ...
            <div className="alert-card-body">
                ...
                <ul className="risk-factors-list">
                  {scdEarlyOnset.map((member) => (
                    <li key={member.id} className="risk-factor-item">
                      ...
                      {member.name} ({member.relationship}) - Diagnosed at age{" "}
              ...
              <div
                className="alert-recommendation"
                style={{ borderLeftColor: "#14b8a6" }}
              >
                <p className="recommendation-title">Clinical Recommendation:</p>
                <p className="recommendation-text">
                  Genetic counseling and preventive cardiac screening
                  recommended for patient and immediate family members.
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div key={member.id} className="member-card-component">
                    <div className="member-card-header-component">
                      <div className="member-header-content">
                        ...
                        <div className="member-info-text">
                          ...
                          <p className="member-relation-text">
                            {member.relationship}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="member-card-body-component">
                      ...
                      <div className="member-detail-row-component">
                        ...
                        <span
                          className={`member-status-badge ${member.health_status &&
                      ...
                      {member.diagnosis_age && (
                        <div className="member-detail-row-component">
                          <span className="detail-label-component">
                            Diagnosed:
                          </span>
                          <span className="detail-value-component">
                            Age {member.diagnosis_age}
                          </span>
                        </div>
                      )}
                    ...
                    <div className="record-header">
                      <div className="record-header-content">
                        <h3 className="record-diagnosis">
                          {record.diagnosis || record.record_type}
                        </h3>

                        <p className="record-date">
                          {record.diagnosis_date
                            ? format(
                              new Date(record.diagnosis_date),
                              "MMM d, yyyy"
                            )
                            : record.created_at
                              ? format(
                        ...
                        <div className="record-detail-box">
                          ...
                          <div className="medications-list">
                            {record.medications.map((med, idx) => (
                              <span key={idx} className="medication-badge">
                                {med}
                              </span>
                            ))}
                          </div>
