// src/Prototype/Pages/Medications.jsx
// @ts-nocheck
...
export default function Medications() {
  ...
  async function fetchMedications() {
    ...
    try {
      ...
      if (viewMode === "patient") {
        ...
        query = query.eq("patient_id", currentUser.uid);
      } else {
        // Show medications prescribed by this doctor
        query = query.eq("doctor_id", currentUser.uid);
      }

      // Sort by start_date (newest first) - stored as text, but still okay for ISO-like strings
  ...
  const handleAddMedication = async (e) => {
    ...
    if (!currentUser) {
      alert("You must be logged in to manage medications.");
      return;
    }
    ...
    try {
      ...
      const newMedication = {
        ...
        prescribed_by:
          viewMode === "doctor"
            ? currentUser.uid
            : formData.prescribed_by || null,
        diagnosis: formData.diagnosis || null,
        duration_days: formData.duration_days
          ? parseInt(formData.duration_days)
          : null,
        created_at: editingMed?.created_at || now,
        updated_at: now,
      };
  ...
  const handleTimesChange = (numTimes) => {
    const safeNum = isNaN(numTimes) || numTimes < 1 ? 1 : numTimes > 6 ? 6 : numTimes;
    const times = [];
    const baseHour = 9; // Start at 9 AM
    const interval = Math.floor(12 / safeNum); // Spread across ~12 hours
                  ...
                  <div className="medication-card-body">
                    ...
                    {med.refill_date && (
                      <div className="medication-refill-alert">
                        <Bell size={14} />
                        <span>
                          Refill due: {formatDate(med.refill_date)}
                        </span>
                      </div>
                    )}
                    ...
                    {med.side_effects && (
                      <div className="medication-instructions">
                        <strong>Known Side Effects:</strong>
              ...
              <div className="form-content">
                ...
                <div className="form-field">
                  ...
                  <div className="times-grid">
                    {(formData.administration_times || []).map(
                      (time, index) => (
                        <input
                          key={index}
                          type="time"
                ...
                {isDoctor && (
                  <>
                    <div className="form-field">
                      <label
                        htmlFor="prescribed_for_patient"
                        className="form-label"
                      >
                        Patient ID *
                      </label>
